...
def _match_by_span_positions(pdf_bytes: bytes) -> tuple[str, str]:
    ...
    try:
        ...
        station_spans: list[tuple[float, str]] = []
...
def _match_by_pinyin_blocks(pdf_bytes: bytes, stations_cn: list[str]) -> tuple[str, str]:
...
def _match_by_pinyin_lines(lines: list[str], stations_cn: list[str]) -> tuple[str, str]:
    """通过纯文本拼音行定位发站和到站。"""
